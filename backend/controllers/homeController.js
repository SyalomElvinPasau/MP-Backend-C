...
export async function renderHomePage(request, response) {
    ...
    // Split HTML at {{POSTS}}
    const [beforePosts, afterPosts] = html.split("{{POSTS}}");
    ...
    // Render admin button in header section
    let headerHTML = beforePosts.replace(
        "{{ADMIN_BUTTON}}",
        user.role === "admin"
            ? `
            <a href="/user-list" class="menu-item user-list">
    ...
    const postFormHTML = `
        <form id="commentForm"
            ...
            <div class="actions">
                <button id="submitBtn" type="submit" class="btn pill">Publish Comment</button>
                <button id="clearBtn" type="button" class="btn pill ghost">Clear Form</button>
            </div>
        </form>
    `;

    headerHTML = headerHTML.replace("{{FORM}}", postFormHTML);

    // Send header part now
    response.write(headerHTML);

    // Stream each post chunk-by-chunk
    for (const post of posts) {
        ...
        const postChunk = `
            <article class="post">
                ...
                <div class="activity">
                    <div class="like">
                        <img src="${
                            liked ? "/icon/heart-filled.png" : "/icon/heart.png"
                        }" class="like-btn" data-post-id="${post.id}">
                        <p>${post.likes.length}</p>
                    </div>
        ...
        response.write(postChunk); // Send chunk
    }

    // End the HTML document
    response.end(afterPosts);
}
